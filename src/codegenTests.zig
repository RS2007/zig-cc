...
test "function and integer args, float to uint" {
    var arena = std.heap.ArenaAllocator.init(std.heap.page_allocator);
    const allocator = arena.allocator();
    defer arena.deinit();
    const cFileWriter = (try std.fs.cwd().createFile("./cFiles/C/floatToUInt.c", .{})).writer();
    const sFileWriter = (try std.fs.cwd().createFile("./cFiles/S/floatToUInt.s", .{})).writer();
    const programStr =
        \\ unsigned int add(double a,unsigned int b){ return a + b;}
        \\ int main(){
        \\    double k = add(2.0,3);
        \\    if(k == 5.0){return 0;}
        \\    return 1;
...
test "conversion from float to long" {
    var arena = std.heap.ArenaAllocator.init(std.heap.page_allocator);
    const allocator = arena.allocator();
    defer arena.deinit();
    const cFileWriter = (try std.fs.cwd().createFile("./cFiles/C/floatToSLong.c", .{})).writer();
    const sFileWriter = (try std.fs.cwd().createFile("./cFiles/S/floatToSLong.s", .{})).writer();
    const programStr =
        \\ int main() {
        \\     double f = 1234567890.1234567890;
        \\     long l = f;
        \\ 
        \\     if (l == 1234567890L) {
        \\         return 0;
...
test "conversion from uint to double" {
    var arena = std.heap.ArenaAllocator.init(std.heap.page_allocator);
    const allocator = arena.allocator();
    defer arena.deinit();
    const cFileWriter = (try std.fs.cwd().createFile("./cFiles/C/uIntToDouble.c", .{})).writer();
    const sFileWriter = (try std.fs.cwd().createFile("./cFiles/S/uIntToDouble.s", .{})).writer();
    const programStr =
        \\    int main() {
...
test "simple char test" {
    var arena = std.heap.ArenaAllocator.init(std.heap.page_allocator);
    const allocator = arena.allocator();
    defer arena.deinit();
    const cFileWriter = (try std.fs.cwd().createFile("./cFiles/C/charSimple.c", .{})).writer();
    const sFileWriter = (try std.fs.cwd().createFile("./cFiles/S/charSimple.s", .{})).writer();
    const programStr =
        \\ int main() { return 'a'; }
    ;
    const l = try lexer.Lexer.init(allocator, @as([]u8, @constCast(programStr)));
...
test "a simple printf" {
    var arena = std.heap.ArenaAllocator.init(std.heap.page_allocator);
    const allocator = arena.allocator();
    defer arena.deinit();
    const cFileWriter = (try std.fs.cwd().createFile("./cFiles/C/printf.c", .{})).writer();
    const sFileWriter = (try std.fs.cwd().createFile("./cFiles/S/printf.s", .{})).writer();
    const programStr =
...
test "strlen" {
    ...
    const allocator = arena.allocator();
    defer arena.deinit();
    const cFileWriter = (try std.fs.cwd().createFile("./cFiles/C/strlen.c", .{})).writer();
    const sFileWriter = (try std.fs.cwd().createFile("./cFiles/S/strlen.s", .{})).writer();
    const programStr =
        \\ unsigned long strlen(char *format);
...
test "strcat strcmp" {
    ...
    const allocator = arena.allocator();
    defer arena.deinit();
    const cFileWriter = (try std.fs.cwd().createFile("./cFiles/C/strcatcmp.c", .{})).writer();
    const sFileWriter = (try std.fs.cwd().createFile("./cFiles/S/strcatcmp.s", .{})).writer();
    const programStr =
